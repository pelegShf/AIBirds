...
def generate_state(img_dir, img_dir_zoomed, model='./vision/best.pt'):
    ...
    sling_found, sling, state, bird_on_sling = create_state('./filename.txt')
    os.remove('./filename.txt')
    print(f'first sling: {sling["x"]} {sling["y"]} {sling["w"]} {sling["h"]}')
    print(sling_found)
    if not sling_found:
        x, y = find_sling_yolo(img_dir, model)
        return state, x, y, bird_on_sling
    else:
        return state, (sling["x"] + sling["w"] * OFFSETX), (sling["y"] + sling["w"] * OFFSETX), bird_on_sling
...
def create_state(file_path):
    ...
    bird_on_sling = lines[-1].strip()
    print(f"bird_on_sling: {bird_on_sling}")
    ...
    numerical_array = np.zeros((num_cells_y, num_cells_x)).astype(np.uint8)
